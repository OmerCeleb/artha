...
interface TransactionContextType {
    ...
    addGoal: (goalData: Omit<Goal, 'id' | 'userId' | 'startDate' | 'completed'>) => void;
...
export const TransactionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        if (user) {
            ...
            // If no goals exist, load initial sample goals
            if (loadedGoals.length === 0) {
                const initialGoals = getInitialGoals(user.id);
